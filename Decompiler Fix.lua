...
local function main()
    ...
    for _, v in next, selection.List do
        if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
            ...
            -- Sanitize name for Android file systems
            local fileName = ("%s_%s.txt"):format(env.parsefile(v.Obj.Name):gsub("[^%w%s_]+", ""), tostring(game.PlaceId))
            
            -- standard writefile for Android executors
            if env.writefile then
                env.writefile(fileName, source)
                print("Saved to: " .. fileName)
            end
            
            -- If your Dex version uses a custom UI Prompt (Lib.SaveAsPrompt):
            if Lib.SaveAsPrompt then
    ...
    for _, v in next, selection.List do
        if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
            ...
            if success and type(bytecode) == "string" then
                local fileName = ("%s_%s_bytecode.txt"):format(env.parsefile(v.Obj.Name):gsub("[^%w%s_]+", ""), tostring(game.PlaceId))
                
                if env.writefile then
                    env.writefile(fileName, bytecode)
                    print("Bytecode saved to: " .. fileName)
                end
