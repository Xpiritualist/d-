...
local function main()
	...
	-- Modified Context Menu Registration for Android 2026
-- This fixes the "Empty com.roblox.client" folder issue.
    ...
    OnClick = function()
        for _, v in next, selection.List do
            if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
                ...
                -- 1. Sanitize name for Android (No special chars)
                local cleanName = env.parsefile(v.Obj.Name):gsub('[%\\%/%:%*%?%"%<%>%|]', "_")
                local fileName = ("%s_%s.txt"):format(cleanName, tostring(game.PlaceId))
                
                -- 2. Try Public Paths first, then fallback to internal workspace
                local paths = {
                    "../../../../Download/" .. fileName, -- Public Download folder
                    "../../../../Documents/" .. fileName, -- Public Documents folder
                    fileName                              -- Default internal workspace
                }
                
                local saved = false
                if env.writefile then
                    for _, path in ipairs(paths) do
                        local ok = pcall(function() env.writefile(path, source) end)
                        if ok then
                ...
                if not saved and Lib.SaveAsPrompt then
        ...
        for _, v in next, selection.List do
            if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
                ...
                if success and type(bytecode) == "string" then
                    local cleanName = env.parsefile(v.Obj.Name):gsub('[%\\%/%:%*%?%"%<%>%|]', "_")
                    local fileName = ("%s_%s_bytecode.txt"):format(cleanName, tostring(game.PlaceId))
                    ...
                    if env.writefile then
                        for _, path in ipairs(paths) do
                            local ok = pcall(function() env.writefile(path, bytecode) end)
                            if ok then
                                print("Bytecode saved to: " .. path)
                                saved = true
                                break
                            end
                        end
